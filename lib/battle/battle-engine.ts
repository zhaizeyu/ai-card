...
function act(actor: Fighter, target: Fighter, log: string[]) {
  ...
  if (actor.cooldown === 0 && shouldTrigger(actor, target, actor.card.trigger)) {
    executeSkill(actor, target, log)
    actor.cooldown = actor.card.cooldown
    return
  }
...
function executeSkill(actor: Fighter, target: Fighter, log: string[]) {
  ...
  const power = actor.card.effectPower
  ...
  if (effect === "regen") {
    actor.statuses = addStatus(actor.statuses, { name: "regen", turns: 3, power })
  ...
  if (effect === "chain") {
    const first = applyDamage(target, calculateDamage(actor.card, target.card, effect), log, actor)
    const second = applyDamage(target, calculateDamage(actor.card, target.card, effect), log, actor)
    log.push(`${actor.card.name} 使用【${actor.card.skillName}】连续攻击 ${target.card.name}，造成 ${first + second} 点总伤害。`)
    return
  }

  if (effect === "pierce") {
    const damage = applyDamage(target, calculateDamage(actor.card, target.card, effect, -target.card.def), log, actor, {
      bypassShield: true,
    })
  ...
  if (effect === "execute") {
    ...
    const damage = applyDamage(target, calculateDamage(actor.card, target.card, effect) + missingHpBonus, log, actor)
  ...
  const damage = applyDamage(target, calculateDamage(actor.card, target.card, effect, defModifier), log, actor)
  log.push(`${actor.card.name} 使用【${actor.card.skillName}】攻击 ${target.card.name}，造成 ${damage} 点伤害。`)

  if (effect === "burn") {
    target.statuses = addStatus(target.statuses, { name: "burn", turns: 2, power })
    log.push(`${target.card.name} 被点燃。`)
  } else if (effect === "armor_break") {
    target.statuses = addStatus(target.statuses, { name: "armor_break", turns: 2, power: 2 })
    ...
    target.statuses = addStatus(target.statuses, { name: "poison", turns: 3, power })
    log.push(`${target.card.name} 中毒了。`)
  } else if (effect === "mark") {
    target.statuses = addStatus(target.statuses, { name: "mark", turns: 3, power })
