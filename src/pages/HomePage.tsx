...
export default function HomePage() {
  const latestCards = useGameStore((state) => state.homeCards)
  const loadHomeCards = useGameStore((state) => state.loadHomeCards)
    ...
    <main>
        ...
        <div className="mx-auto grid min-h-[540px] max-w-6xl items-center gap-10 px-4 py-16 md:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link className="inline-flex h-10 items-center rounded-md border border-ink/15 bg-white px-4 text-sm font-semibold" to="/gallery">
                查看图鉴
              </Link>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12">
        <div className="mb-6 flex items-end justify-between">
          ...
          <Link to="/gallery" className="text-sm font-semibold text-ink/70">
            全部
          </Link>
        </div>
        {error ? <p className="mb-4 text-sm font-semibold text-ember">{error}</p> : null}
        {latestCards.length ? (
